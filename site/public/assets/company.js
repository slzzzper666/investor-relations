/* 公司頁：依 ?code=2330 載入 company/{code}.json。
...
(function () {
  ...
  function confRow(c) {
    ...
    return '<a class="row" href="detail.html?id=' + encodeURIComponent(c.id) + '">' +
      ...
      '<span class="row-main">' +
        '<span class="row-company">' + esc(c.date.slice(0, 4)) + " 年法說會</span>" +
  ...
  function render(d) {
    ...
    if (fin.industry) {
      eyebrow.push('<a href="index.html?industry=' + encodeURIComponent(fin.industry) + '">' +
        esc(fin.industry) + "</a>");
    }
    var tags = (biz.tags || []).map(function (t) {
      return '<a class="biz-tag" href="group.html?tag=' + encodeURIComponent(t) + '">' +
        esc(t) + "</a>";
    }).join("");
    ...
    var confsHtml = confs.length
      ? '<section class="ledger company-ledger">' + confs.map(confRow).join("") + "</section>"
      : '<p class="prose">尚無收錄的法說會。</p>';
      ...
      '<div class="doc-body">' +
        "<article>" +
          '<section class="doc-section">' +
            "<h2>歷次法說會" +
              '<span class="h2-note">' + confs.length + " 場 · 逐字稿 " + withTr + " 場</span>" +
            "</h2>" + confsHtml +
          "</section>" +
        "</article>" +
        '<div class="doc-side">' +
          P.businessHtml(biz, "") + P.financialsHtml(fin, finCtx) +
        "</div>" +
      "</div>" +
      '<nav class="doc-foot-nav"><a href="index.html">&larr; 返回清單</a></nav>';
  }

  var m = /[?&]code=(\d{4})/.exec(location.search);
  if (!m) {
    fail("缺少公司代號", "網址格式：company.html?code=2330");
    return;
  }
  fetch("company/" + m[1] + ".json", { cache: "no-cache" })
